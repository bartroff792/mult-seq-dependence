...
from .data_funcs import (
    ...
    assemble_fake_binom_general,
    assemble_fake_pois_general,
    generate_llr,
    generate_llr_general,
    check_params,
)
...
def run_mc_synth_sim_tests(
    ...
    for i in main_iter:
        if rejective:
            llr_data, obs_data = generate_llr_general(
                params=params,
                n_periods=n_periods,
                rho=rho,
                hyp_type=hyp_type,
...
def calc_sim_cutoffs(
    ...
    if beta is not None:  # Infinite horizon
        ...
        if n_periods is None:
            ...
            # E[\tilde{\tau}] / \delta <= n
            n_periods = int(
                cutoff_funcs.est_sample_size_general(

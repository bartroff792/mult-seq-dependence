...
def test_base_pvalue_cuts(cut_type):
    m_hyps = 5
    alpha_vec = cutoff_funcs.construct_base_pvalue_cutoffs(
        cut_type=cut_type,
        m_total=m_hyps,
        alpha=0.1,
    )
...
class TestLLRTermMomentsGeneral:
    def test_binom(self):
        ...
        p0 = 0.25
        p1 = 0.66
        params0 = {"n": pd.Series([n, n, n]), "p": pd.Series([p0, p0, p0])}
        params1 = {"n": pd.Series([n, n, n]), "p": pd.Series([p1, p1, p1])}
    ...
    def test_norm_loc_known_var(self):
        ...
        mu0 = 0.0
        mu1 = 0.5
        params0 = {"mu": pd.Series([mu0, mu0, mu0]), "sigma_sq": pd.Series([sigma_sq, sigma_sq, sigma_sq])}
        params1 = {"mu": pd.Series([mu1, mu1, mu1]), "sigma_sq": pd.Series([sigma_sq, sigma_sq, sigma_sq])}
        term_mean = cutoff_funcs.llr_term_mean_general(params0, params1, "norm_loc_known_var")
        assert (term_mean < 0).all()
        alt_term_mean = cutoff_funcs.llr_term_mean_general(params1, params0, "norm_loc_known_var")
        assert (alt_term_mean < 0).all()
...
class TestEstSampleSize:
    def test_binom(self):
        ...
        sample_size = cutoff_funcs.est_sample_size(
            EasyA_vec, EasyB_vec, params0, params1, "binom"
        )
    ...
    def test_norm_loc_known_var(self):
        ...
        params1 = {"mu": pd.Series([mu1, mu1, mu1]), "sigma_sq": pd.Series([sigma_sq, sigma_sq, sigma_sq])}
        sample_size = cutoff_funcs.est_sample_size(
            EasyA_vec, EasyB_vec, params0, params1, "norm_loc_known_var"
        )
        assert sample_size > 5
    ...
    def test_poisson(self):
        ...
        sample_size = cutoff_funcs.est_sample_size(
            EasyA_vec, EasyB_vec, params0, params1, "pois"
        )
        assert sample_size > 5
    ...
    def test_drug(self):
        ...
        params0, params1 = build_simple_drug_params(m_hyps)
        sample_size = cutoff_funcs.est_sample_size(
            EasyA_vec, EasyB_vec, params0, params1, "drug"
        )
        assert sample_size > 5
...
class TestFiniteHorizonCutoffSimulationGeneral:
    def test_binom(self):
        ...
        assert len(weights) == len(
            max_llr
        ), f"weights and max_llr have different lengths"
        mean_cut = np.sum(weights * max_llr) / np.sum(weights)
        var_cut = np.sum(weights * (max_llr - mean_cut) ** 2) / np.sum(weights)
        assert mean_cut > np.sqrt(
            var_cut
        ), f"mean_cut={mean_cut} <= np.sqrt(var_cut)={np.sqrt(var_cut)}"
...
class TestInfiniteHorizonPfdrPfnrPvalueCutoffs:
    def test_no_m0_guess(self):
        ...
        alpha_pfdr = 0.05
        beta_pfnr = 0.2
        alpha_raw = cutoff_funcs.create_fdr_controlled_bh_alpha_indpt(
            alpha_pfdr, m_hyps
        )
        beta_raw = cutoff_funcs.create_fdr_controlled_bh_alpha_indpt(
            beta_pfnr,
            m_hyps,
        )
    ...
    def test_bad_m0_guess(self, m, m0):
        ...
        with pytest.raises(AssertionError):
            alpha_vec, beta_vec = (
                cutoff_funcs.pfdr_pfnr_infinite_horizon_pvalue_cutoffs(
                    alpha_raw,
                    beta_raw,
                    alpha_pfdr,
                    beta_pfnr,
...
class TestFiniteHorizonCutoffs:
    ...
    def test_poisson(self):
        ...
        alpha_levels = cutoff_funcs.create_fdr_controlled_bl_alpha_indpt(
            0.1, m_hyps=m_hyps
        )
        np.random.seed(42)
    ...
    def test_binom(self):
        ...
        p1 = 0.66
        params0 = {
            "n": pd.Series(np.repeat([n], m_hyps)),
    ...
    def test_norm_loc_known_var(self):
        ...
        m_hyps = 5
        params0 = {
            "mu": pd.Series(np.linspace(mu0, mu1, m_hyps)),
            "sigma_sq": pd.Series(np.repeat([sigma_sq], m_hyps)),
        }
        params1 = {
            "mu": pd.Series(np.linspace(mu1, mu0 + mu1, m_hyps)),
            "sigma_sq": pd.Series(np.repeat([sigma_sq], m_hyps)),
        }
        alpha_levels = cutoff_funcs.create_fdr_controlled_bl_alpha_indpt(
            0.1, m_hyps=m_hyps

...
The main functions are:
    ...
    modular_sprt_test: Modular SPRT. The main function for running SPRTs.
...
def modular_sprt_test(llr, A_vec, B_vec, record_interval=100, stepup=False, 
    ...
    Returns:
        ...
        termination_time_series (pd.DataFrame): record of terminations by time step
        ...
        cutoff_output (pd.DataFrame): Cutoff values
            alpha:
            beta:
            A:
            B:
    ...
    """
    if isinstance(llr, pd.DataFrame):
    ...
    # other setup
    termination_level = pd.DataFrame({'step':np.NaN, 'rejLevel':np.NaN, 'accLevel':np.NaN}, index=hyp_names)
...
#    raise ValueError()
    ...
    naive_barrier_tripped = (num_crossing_barrier >= np.arange(1, 1 + len(num_crossing_barrier))).astype(bool)
